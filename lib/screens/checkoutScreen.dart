...
class _CheckoutScreenState extends State<CheckoutScreen> {
  int? selectedPaymentIndex; // Use int? for nullable int
  ...
  onChanged: (bool value) {
    setState(() {
      if (value) {
        ...
        selectedPaymentIndex = -1; // Use -1 to indicate no selection
      }
    });
  },
),
...
class PaymentCardSelection extends StatelessWidget {
  ...
  final Function(bool) onChanged; // Callback function to handle selection
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () {
        onChanged(!isSelected); // Toggle the selection when tapped
      },
      child: Padding(
        padding: const EdgeInsets.symmetric(horizontal: 20),
        child: Container(
          ...
          decoration: ShapeDecoration(
            shape: RoundedRectangleBorder(
              ...
              side: BorderSide(
                color: isSelected ? AppColor.orange : AppColor.placeholder.withOpacity(0.25),
              ),
            ),
            color: isSelected ? AppColor.orange : AppColor.placeholderBg,
          ),
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
